 ...
 * @param {ParamsUserTopItems} payload - Params for get user's top artists and tracks
 * @returns {Promise<SpotifyUserTopArtists>} User's Top Artists and Tracks
 */
export const getTopArtists = async (payload: ParamsUserTopItems): Promise<SpotifyUserTopArtists> => {
  const { time_range = "short_term"} = payload;
  const params = {
    time_range
  };
  try {
